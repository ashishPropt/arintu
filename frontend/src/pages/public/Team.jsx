...
function MemberCard({ m, index, expandedId, onToggle }) {
  const isExpanded = expandedId === m.id;
  const ringColor  = RING_COLORS[index % RING_COLORS.length];
  const bgColor    = BG_COLORS[index % BG_COLORS.length];

  return (
    <div className={`col-span-1 ${isExpanded ? 'sm:col-span-3 lg:col-span-4' : ''}`}>
      {/* Collapsed card */}
      <button
      ...
      >
        ...
        <div>
          ...
          {m.title && <p className="text-xs text-brand-600 mt-0.5 leading-tight">{m.title}</p>}
        </div>
      ...
      {isExpanded && (
        <div className="mt-3 bg-white rounded-2xl border border-brand-200 shadow-md p-6 flex flex-col sm:flex-row gap-6 text-left">
          ...
          <div className="flex-1">
            ...
            {m.title && <p className="text-sm font-medium text-brand-600 mb-3">{m.title}</p>}
            {m.bio ? (
...
export default function Team() {
  ...
  const staff    = members.filter((m) => m.member_type !== 'teacher');
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-14">
      ...
      <p className="text-gray-500 mb-10">
        ...
        Click on any member to learn more.
      </p>
      ...
      {loading ? (
        ...
        <>
          ...
          {staff.length > 0 && (
            <section className="mb-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
                {staff.map((m, i) => (
                  <MemberCard key={m.id} m={m} index={i} expandedId={expandedId} onToggle={toggle} />
                ))}
              </div>
            </section>
          )}
          ...
          {staff.length === 0 && teachers.length === 0 && (
